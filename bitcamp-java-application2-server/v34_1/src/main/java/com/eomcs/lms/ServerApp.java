// v33_4: 클라이언트 요청을 처리하는 부분을 별도의 스레드(실)로 분리하여 실행한다.
  ...
  public void execute() {
    ...
    try (ServerSocket serverSocket = new ServerSocket(this.port)) {
      ...
      while (true) {
        ...
        // // => RequestHandler는 별도의 실행 흐름으로 분리될 수 있는 Thread기능을 상속 받았다.
        // requestHandler.start();

        new RequestHandler(socket, servletContext).start();
        // 스레드를 분리하여 실행시키면 main thread는 즉시 다음 명령을 실행한다.

        // 클라이언트 중에 하나가 서버 종료를 설정했다면, 현재 접속한 클라이언트 요청을 처리하지 않고
        // 즉시 서버 실행을 멈춘다.
        // 주의!
