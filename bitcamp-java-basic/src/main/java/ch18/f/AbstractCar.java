...
// 인터페이스의 일부 메소드를 미리 정의하면, 
// 서브 클래스가 인터페이스의 모든 메소드를 정의해야 하는 부담을 줄일 수 있다.
// 인터페이스에 메소드가 많을 때는 특히 이런방식을 사용한다.
// 즉, 인터페이스를 직접 구현하기 보다는 추상 클래스를 상속받아
// 간접적으로 구현하는 기법을 사용한다.
package ch18.f;
...
public abstract class AbstractCar implements CarSpec{
  ...
  // => 그중에서 인터페이스에 선언된 메소드를 일부를 미리 구현해 주면
