...
// => 예) compute()
// 오버라이딩(overriding)?
// => 상속 받은 메소드를 서브 클래스의 역할에 맞게 재정의 하는 것.
public class Score3 extends Score {
  ...
  // => 반드시 원래의 메소드 시그너처(method signature)와 일치해야 한다.
  // => 메소드 시드너처? 메소드 이름과 리턴 타입, 파라미터 선언을 가리킨다.
  ...
  /*
   * @Override protected void compute() { // 기존 계산 메서드를 호출해서 일단 국,영,수 세 과목의 합계와 평균을 계산한다. // => 재정의
   * 하기 전의 메서드를 호출할 때는 super 이용한다. System.out.println("Score3.compute() 호출됨!"); super.compute();
   * this.sum += this.music + this.art; this.aver = this.sum / 5f; }
   */
}
