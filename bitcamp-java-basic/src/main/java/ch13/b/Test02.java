...
public class Test02 {
  public static void main(String[] args) {
    ...
    // 따라서 음악, 미술 점수를 먼저 설정한 다음에 국, 영, 수점수를 설정하면
    // 함계와 평균이 잘못 계산된다.
    
    // 해결책!
    // 1) 국, 영, 수 점수를 설정하는 setter 에서 compute() 대신에 compute2()를 호출하도록 변경
    // 2) compute()가 음악과 미술 점수를 함께 계산 하도록 변경
    // => 기존에 작성한 클래스를 변경하는 것은 유지보수에 바람직하지 않기 때문에
    //    기존 클래스인 Score를 손대지 않는 조건으로 Score클래스의 메소드를 변경해야 한다.
