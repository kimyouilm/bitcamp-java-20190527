...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // => 개발자가 sort()를 오버라이딩 하는 것을 놓쳤거나 일부러 오버라이딩 하지 않은 것이다.
  ...
  static void display(Sorter sorter, int[] values) {
    ...
    // Sorter는 추상 클래스이기 때문에 인스턴스를 생성할 수 없다. 따라서 Sorter의 인스턴스가
    // 넘어올 수는 없는 것이다.
    // 따라서 정렬을 수행하려면 Sorter에 정의된 sort()를 호출할 것이다.
    // 여기서 문제가 발생한다.
    // BubbleSort나  QuickSort클래스는 sort()를 오버라이딩 하였다.
    // 그러나, MergeSort는 sort() 오버라이딩 하지 않았다.
    // 따라서 파라미터로 넘어오는 객체가 MergeSort라면
    // 다음 메소드를 호출하는 것은 아무런 의미가 없을 것이다.
    sorter.sort(values);
    //해결책?
    // => Sorter에 서브 클래스들이 sort() 메소드를 반드시 구현하도록 강제하라!
