...
public class Test02 {
  public static void main(String[] args) {
    ...
    // 클래스에 대해 래버런스 변수를 만들때는 클래스를 로딩하 않는다.
    My2 temp;
    Scanner keyScan = new Scanner(System.in);
    System.out.print(">>");
    // 사용자가 입력한 값 중에서 Line fed(LF, 0x0A)코드를 읽을 때까지 리턴하지 않는다.
    keyScan.nextLine();
    
    // 다음과 같dl My2 파일을 method area영역에 로딩한다. 이것을 테스트하려면 이 클래스를 실행하기 전에My2.class 파일을
    // 살세하가. 그리고 이클래스를ㄹ 실행하려면 위의 코드를 실핼할 때는 오류가 방생하지 않는다.
    // 그러나 아래의 코드를 실행할 때 My2.class가 없다고 오류가 발생한다.
