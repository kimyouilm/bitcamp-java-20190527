// hash code 응용 II Wrapper 클래스를 key객체로 사용하기
package ch15;
...
public class Test10_1 {
  public static void main(String[] args) {
    ...
    // key로 사용할 객체를 준비한다.
    // Integer k1 = Integer.valueOf(101);
    // Integer k2 = Integer.valueOf(102);
    // Integer k3 = Integer.valueOf(103);
    Integer k1 = new Integer(101);
    Integer k2 = new Integer(102);
    Integer k3 = new Integer(103);

    // 위에서 준비한 key객체를 가지고 Student 객체를 보관한다.
    map.put(k1, new Student("홍길동", 20, false));
    map.put(k2, new Student("임꺽정", 30, true));
    map.put(k3, new Student("유관순", 17, true));

    // put(Object key, Object value)
    // => put() 메소드는 key로 넘겨받은 객체에 대해 hashCode()를 호출하여
    // 정수값을 얻는다.
    // => 그렇게 리턴 받은 정수 값(hashCode)를 사용하여 Student 객체(의 주소)를 저장할 위치를 계산한다.
    // => 그런 후 그 위치에 해당하는 배열(배열로 관리한다면)에 저장한다.

    // 다음과 같이 int를 key로 사용할 수 있다.
    // => key값으로 int를 넘겨준다?
    // 내부적으로 auto-boxing을 수행하여 Integer객체를 만든다.
    // 그리고 그 객체를 넘겨주는 것이다.
    map.put(104, new Student("안중근", 24, true));
    map.put(105, new Student("윤봉길", 22, false));

    // 객체를 저장할 때 사용한 key로 다시 객체를 꺼내보자!
    ...
    // 결론!
    // => k2와 k6는 다른 객체지만, hashCode()의 리턴 값이 같고,
    //    equals()의 리턴 값이 true이기 때문에 두 객체는 같은 key로 간주 된다.
    ...
    // 그러나 Integer클래스 등의 Wrapper 클래스와 String클래스는 
    // Object 클래스에서 상속 받은 hashCode()와 equals()를 오버라이딩 했기 때문에
    // 인스턴스가 다르더라도 인스턴스 필드의 값이 같으면 
    // hashCode()의 리턴값이 같고 equals()가 true를 리턴한다.
    // 그래서 wrapper 클래스와 String클래스는 HashMap/Hashtable의
    // key로 사용할 수 있는 것이다.
    // 실무에서도 String이나 primitive type(결국 wrapper 객체로 전환되기 때문)을 key로 자주 사용한다.
  }
