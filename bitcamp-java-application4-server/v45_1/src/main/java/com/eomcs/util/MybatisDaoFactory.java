    ...
    return (T) Proxy.newProxyInstance(clazz.getClassLoader(), new Class[] {clazz},
        (Object proxy, Method method, Object[] args) -> {
          ...
          try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            // => int라면 insert()/update()/delete() 을 호출한다.

            if (retType == List.class) {
              // 리턴 타입이 List일 경우 select문을 요청하는 것이다.
              ...
              // 리턴타입이 int나 void일 경우 insert/update/delete문을 실행하는것이다.
              // sqlSession 에서는 insert()/update()/delete() 중 아무거나 호출해도 똑같다.
              return (args == null) ? sqlSession.insert(sqlId) : sqlSession.insert(sqlId, args[0]);
            } else {
