...
public class App implements Servlet {
  ...
  public void init(ServletConfig config) throws ServletException {
    this.config = config;
    // 톰켓 서버가 이 객체를 사용하기 전에 이 객체가 작업에 필요한 자원들을 준비할 수 있도록
    // 이 메소드를 호출한다.
  ...
  public void service(javax.servlet.ServletRequest request, javax.servlet.ServletResponse response)
    ...
    // 클라이언트 요청이 들어올 때 마다 톰캣 서버가 호출하는 메소드 이다.

    // 클라이언트가 요청한 정보를 자세하게 뽑기 위해 파라미터 객체를 원래의 타입으로 형변환한다.
    // ServletRequest의 서브 인터페이스임
  ...
  @Override
  public void destroy() {
    // 톰캣 서버가 종료되기 전에 이 객체가 준비한 자원을 해제할 수 있도록 이 메소드를 호출한다.

  }

  @Override
  public String getServletInfo() {
    // 톰캣 서버의 관리자 페이지에 애플리케이션에 대해 간단한 소개를 출력하기 위해
    // 이 메소드를 호출한다. 즉 이 메소드의 리턴값을 관리자 페이지에 출력한다.
    return "수업관리 시스템";
  }

  @Override
  public ServletConfig getServletConfig() {
    // 이 객체를 실행하면서 이 객체의 실행 환경 정보를 알고 싶을 때 이 메소드드를 호출한다.
    // 리턴 값은 init()에서 받은 파라미터 값이다.
    // 따라서 init() 메소드에서 파라미터 값을 잘 보관해 둬야한다.
    return null;
  }
